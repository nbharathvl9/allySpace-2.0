...
export default function MemberTaskModel({ isOpen, onClose, subteam }) {
  ...
  const handleSubmit = async () => {
    if (!myTaskData?.taskId) return alert("No active task assigned to you.");
    
    setIsSubmitting(true);
    try {
      await api.post("/task/respond", {
        taskId: myTaskData.taskId,
        status,
        message: responseMsg,
      });
      alert("Mission report updated!");
          ...
          <div style={{ flex: 2, position: "relative", borderRight: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <div style={{ position: "absolute", top: "25px", left: "30px", zIndex: 10 }}>
              ...
              <p style={{ color: "#64748b", fontSize: "13px", marginTop: "4px" }}>
                Tactical View • {members.length} Agents Active
              </p>
            </div>
                ...
                <div className="central-hologram">
                  {hoveredMember ? (
                    <div className="fade-in" key={hoveredMember._id}>
                      ...
                      {hoveredMember.responseMessage && (
                        <div style={{marginTop: "8px", fontSize: "11px", color: "#60a5fa", fontStyle: "italic", padding: "0 10px"}}>
                          "{hoveredMember.responseMessage}"
                        </div>
                      )}
                    </div>
                    ...
                    >
                      <div 
                        ...
                        style={{ 
                          ...
                          borderColor: isMe ? "#4ade80" : undefined,
                          boxShadow: isMe ? "0 0 25px rgba(74, 222, 128, 0.3)" : undefined
                        }}
                      />
                      <div className="seat-tooltip" style={{ color: isMe ? "#4ade80" : "white" }}>
                        {isMe ? "YOU" : `@${m.userName}`}
                      </div>
                    </div>
                  );
            ...
            {myTaskData && myTaskData.taskId ? (
              <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: "11px", textTransform: "uppercase", letterSpacing: "1px", color: "#60a5fa", fontWeight: "700" }}>
                    Assigned Objective
                  </span>
                  <h4 style={{ color: "#fff", fontSize: "16px", margin: "8px 0 5px 0" }}>
                    {myTaskData.assignedTask}
                  </h4>
                  <p style={{ fontSize: "13px", color: "#cbd5e1", lineHeight: "1.5", margin: 0 }}>
                    {myTaskData.description || "Execute orders as directed."}
                  </p>
                  <div style={{ marginTop: "12px", fontSize: "12px", color: "#94a3b8", display: "flex", alignItems: "center", gap: "6px" }}>
                    <span>🕒 Deadline:</span>
                    <span style={{ color: "#e2e8f0" }}>{formatDate(myTaskData.deadline)}</span>
                  </div>
                </div>
                ...
                <div className="modal-input-block" style={{ flexGrow: 1 }}>
                  ...
                  <textarea
                    ...
                    style={{ 
                      width: "100%", height: "100%", minHeight: "120px", padding: "16px", borderRadius: "12px", 
                      background: "rgba(255,255,255,0.05)", color: "white", 
                      border: "1px solid rgba(255,255,255,0.1)", outline: "none",
